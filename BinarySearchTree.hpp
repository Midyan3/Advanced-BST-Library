...
public:
  ...
  BinarySearchTree(std::vector<T>& nums);  
  ...
  void inorderTraverse(std::shared_ptr<BinaryNode<T> > Node  ,std::vector<std::shared_ptr<BinaryNode<T>>> &container) const;

  bool isBalanced() const;

  bool isBalancedHelper(std::shared_ptr<BinaryNode<T>> subtree_ptr) const;

  bool isHelpComplete(std::shared_ptr<BinaryNode<T>> subtree_ptr) const; 
  bool isComplete() const;

  bool isFullHelper(std::shared_ptr<BinaryNode<T>> root) const;

  bool isFull() const;

  std::string HelperForDirect(std::shared_ptr<BinaryNode<T>> root, const T&  p, const T&  q);
  
  std::string getDirectSupervisor(Employee &p, Employee &q);
private:
