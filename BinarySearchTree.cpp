...
template <class T>
void BinarySearchTree<T>::inorderTraverse(std::shared_ptr<BinaryNode<T>> Node ,std::vector<std::shared_ptr<BinaryNode<T>>> &container) const
{
  if (Node == nullptr)
  {
    return;
  }
  inorderTraverse(Node->getLeftChildPtr(), container);
  container.push_back(Node);
  inorderTraverse(Node->getRightChildPtr(), container);
}

template <class T>
void BinarySearchTree<T>::makeIncreasingBST(){
  std::vector<std::shared_ptr<BinaryNode<T>>> container;
  inorderTraverse(root_ptr_, container);
   for (int i = 0; i < container.size() - 1; i++)
  {
    container[i]->setRightChildPtr(container[i + 1]);
    container[i]->setLeftChildPtr(nullptr);
  }
  container[container.size() - 1]->setRightChildPtr(nullptr);
  container[container.size() - 1]->setLeftChildPtr(nullptr);
  root_ptr_ = container[0];
...
/** Helper function for isBalanced method
  @param subtree_ptr a pointer to the subtree to check for balanced property
  @return true if the subtree is balanced, false otherwise
**/
template <class T>
bool BinarySearchTree<T>::isBalancedHelper(std::shared_ptr<BinaryNode<T>> subtree_ptr) const
{
  if (subtree_ptr == nullptr)
    return true;

  // Calculate the heights of the left and right subtrees
  int left_height = getHeightHelper(subtree_ptr->getLeftChildPtr());
  int right_height = getHeightHelper(subtree_ptr->getRightChildPtr());

  // Check if the heights differ by more than 1
  if (std::abs(left_height - right_height) > 1)
    return false;

  // Check if all paths from the root to leaf nodes differ by at most one
  if (!isBalancedHelper(subtree_ptr->getLeftChildPtr()) || !isBalancedHelper(subtree_ptr->getRightChildPtr()))
    return false;
...
{
  return isHelpComplete(root_ptr_);
}

/** Helper function for isComplete method
  @param subtree_ptr a pointer to the subtree to check for complete property
  @return true if the subtree is complete, false otherwise
**/
template <class T>
bool BinarySearchTree<T>::isHelpComplete(std::shared_ptr<BinaryNode<T>> subtree_ptr) const
{
  if (subtree_ptr == nullptr)
    return true;

  std::queue<std::pair<std::shared_ptr<BinaryNode<T>>, int>> node_queue;
  node_queue.push({subtree_ptr, 1});

  int curr_level = 1;
  bool hasIncompleteNode = false;
  bool allNodesToLeftHaveTwoChildren = true;

  while (!node_queue.empty())
  {
    auto curr_pair = node_queue.front();
    auto curr_node = curr_pair.first;
    int curr_level = curr_pair.second;
    node_queue.pop();

    // If an incomplete node was found previously, but the current node has any child,
    // then the BST is not complete
    if (hasIncompleteNode && (curr_node->getLeftChildPtr() || curr_node->getRightChildPtr()))
      return false;

    // If a node has only one child, it must be the left child
    if (curr_node->getLeftChildPtr() && !curr_node->getRightChildPtr())
      return false;

    // If a node doesn't have a left child but has a right child, or
    // if both children are missing, mark it as an incomplete node
    if (!curr_node->getLeftChildPtr() || !curr_node->getRightChildPtr())
      hasIncompleteNode = true;

    // Check condition for level h-1 nodes
    if (curr_level > 1)
    {
      if (curr_node->getLeftChildPtr() && !curr_node->getRightChildPtr())
        allNodesToLeftHaveTwoChildren = false;
    }

    if (curr_node->getLeftChildPtr())
      node_queue.push({curr_node->getLeftChildPtr(), curr_level + 1});

    if (curr_node->getRightChildPtr())
      node_queue.push({curr_node->getRightChildPtr(), curr_level + 1});
  }

  // If the traversal completes without any violation of the conditions,
  // the BST is complete
  return allNodesToLeftHaveTwoChildren;
}

template <class T>
bool BinarySearchTree<T>::isFullHelper(std::shared_ptr<BinaryNode<T>> root) const
{
    // If empty tree
    if (root == nullptr)
        return true;
    ...
    // If leaf node
    if (root->getLeftChildPtr() == nullptr && root->getRightChildPtr() == nullptr)
        return true;
...
{
  return isFullHelper(root_ptr_);
}

template <class T>
BinarySearchTree<T>::BinarySearchTree(std::vector<T>& nums){

}

template <class T>
std::string BinarySearchTree<T>::getDirectSupervisor(Employee &p, Employee &q){
  return HelperForDirect(root_ptr_, p, q);
}

template <class T>
std::string BinarySearchTree<T>::HelperForDirect(std::shared_ptr<BinaryNode<T>> root, const T&  p, const T&  q)
{
  if ((root->getItem() > q) && (root->getItem() > p)){
    return HelperForDirect(root->getLeftChildPtr(), p, q);
  }
  if ((root->getItem() < p) && (root->getItem() < q)){
    return HelperForDirect(root->getRightChildPtr(), p, q);
    }
  else{
